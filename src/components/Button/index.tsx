...
export const ButtonIcon = ({ description, icon, onClick }: PropsIcon) => {
    return (
        <button onClick={onClick} className='w-auto flex items-center gap-2 bg-black text-white rounded-3xl border-2 dark:bg-[#63e] h-full px-6 py-2 cursor-pointer active:scale-[1.1] transition-all duration-300'>
            {icon}
            <span className='text-xs font-bold'>{description}</span>
        </button>
    )
